...
**Опрос на типаж**
Входные - сопостовляется complete_properties в таблице юзера с общим количеством элементов в таблице user_profiles. Если соответствует - шаг завершается. Если меньше - ищется необработанный элемент с типом "типаж". в базе ищется вопрос на типаж, для которого не найден ответ пользователя.
